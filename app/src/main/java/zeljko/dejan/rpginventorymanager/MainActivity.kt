...
class MainActivity : AppCompatActivity() {
    ...
    private fun populateCategorySpinner() {
        lifecycleScope.launch {
            ...
            categories.addAll(Inventory.database.itemDao().getAllCategories())
    ...
    private fun performSearch(query: String) {
        lifecycleScope.launch {
            val filteredItems = withContext(Dispatchers.IO) {
                Inventory.database.itemDao().searchItemsThatContainQuery("%$query%")
            }
    ...
    private fun loadDefaultItems() {
        lifecycleScope.launch {
            val items = withContext(Dispatchers.IO) {
                Inventory.database.itemDao().getAllItems()
            }
        ...
        lifecycleScope.launch {
            val sortedAndFilteredItems = withContext(Dispatchers.IO) {
                val sortedItems = when (sortOption) {
                    "A -> Z" -> Inventory.database.itemDao().getItemsSortedByNameAsc()
                    "Z -> A" -> Inventory.database.itemDao().getItemsSortedByNameAsc().reversed()
                    else -> Inventory.database.itemDao().getAllItems()
                }
